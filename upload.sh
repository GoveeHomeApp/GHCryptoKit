...
#修改spec文件tag
#获取版本号并显示
VersionString=`grep -E "s.version.*=" GHCryptoKit.podspec`
...
LineNumber=`grep -nE 's.version.*=' GHCryptoKit.podspec | cut -d : -f1`
sed -i "" "${LineNumber}s/${VersionNumber}/${NewVersionNumber}/g" GHCryptoKit.podspec
echo -e "\n${Default}================================================"
